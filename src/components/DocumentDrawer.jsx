...
export default function DocumentDrawer({
  ...
  onExportDocx,
  isExporting,
  activeChapterTab = 'cover'
}) {
  const [activeTab, setActiveTab] = useState(activeChapterTab);
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-4xl h-full bg-[#171717] border-l border-[#2e2e2e] shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-right duration-300">
        {/* Drawer Header */}
        <div className="h-16 px-6 border-b border-[#2e2e2e] bg-[#141414] flex items-center justify-between shrink-0">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Times New Roman 12 • 1.5 Spacing • 1.5" Binding
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              onClick={handlePrint}
              className="px-3 py-1.5 rounded-xl bg-[#252525] hover:bg-[#303030] border border-[#383838] text-slate-200 font-semibold text-xs flex items-center space-x-1.5 transition"
            ...
            <button
              ...
              className="px-3.5 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs flex items-center space-x-1.5 shadow-md shadow-amber-500/20 transition disabled:opacity-50"
            >
              <FileDown className="w-3.5 h-3.5" />
              <span>{isExporting ? 'Inapakua...' : 'Pakua Word (.docx)'}</span>
            </button>
        ...
        {/* Chapter Tabs */}
        <div className="px-6 py-2.5 border-b border-[#2e2e2e] bg-[#1a1a1a] flex items-center space-x-1.5 overflow-x-auto text-xs shrink-0">
          <TabBtn id="cover" label="Cover Page" active={activeTab === 'cover'} onClick={() => setActiveTab('cover')} />
          <TabBtn id="prelim" label="Preliminaries (i-vi)" active={activeTab === 'prelim'} onClick={() => setActiveTab('prelim')} />
          <TabBtn id="chap1" label="Chapter 1 (Org)" active={activeTab === 'chap1'} onClick={() => setActiveTab('chap1')} badge={chapters.find(c => c.id === 'chap1')?.sections?.length || 0} />
          <TabBtn id="chap2" label="Chapter 2 (Tasks)" active={activeTab === 'chap2'} onClick={() => setActiveTab('chap2')} badge={chapters.find(c => c.id === 'chap2')?.sections?.length || 0} />
          <TabBtn id="chap3" label="Chapter 3 (Skills)" active={activeTab === 'chap3'} onClick={() => setActiveTab('chap3')} badge={chapters.find(c => c.id === 'chap3')?.sections?.length || 0} />
          <TabBtn id="chap4" label="Chapter 4 (Gaps)" active={activeTab === 'chap4'} onClick={() => setActiveTab('chap4')} badge={chapters.find(c => c.id === 'chap4')?.sections?.length || 0} />
          <TabBtn id="chap5" label="Chapter 5 (Recs)" active={activeTab === 'chap5'} onClick={() => setActiveTab('chap5')} badge={chapters.find(c => c.id === 'chap5')?.sections?.length || 0} />
          <TabBtn id="ref" label="References" active={activeTab === 'ref'} onClick={() => setActiveTab('ref')} />
        </div>

        {/* A4 Paper Document Viewer */}
        <div className="flex-1 p-2 sm:p-4 md:p-6 overflow-y-auto overflow-x-auto bg-[#0d0d0d] flex justify-center">
                    ...
                    <div className="space-y-1.5 font-mono text-xs">
                      {dynamicFigures.map((fig, idx) => (
                        <div key={idx} className="flex justify-between items-baseline">
                          <span className="font-bold text-slate-900">{fig.figureNo}: {fig.title}</span>
                                ...
                                <div className="flex items-center space-x-2">
                                  {currentChap.id === 'chap2' && (
                                    <label className="cursor-pointer opacity-80 hover:opacity-100 px-2 py-0.5 rounded bg-slate-200 hover:bg-amber-100 text-[10px] font-semibold text-slate-700 hover:text-amber-900 flex items-center space-x-1 transition">
                                      ...
                                      <span>{sec.image ? 'Badili Picha' : '+ Picha'}</span>
                                      <input
                                        type="file"
                                        accept="image/*"
                                        className="hidden"
    ...
    >
      ...
      {badge !== undefined && badge > 0 && (
        <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-mono ${
          active ? 'bg-slate-950 text-amber-300' : 'bg-[#2e2e2e] text-amber-400'
        }`}>
          {badge}
        </span>
      )}
